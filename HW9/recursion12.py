...
def fibonacci(n):
    if (n ==0 or n == 1):
        return n
    if (n >= 1):
        fibbonicci = fibonacci(n-1) + fibonacci(n-2)
        return fibbonicci
...
def triangle(rows):
    if (rows == 0 or rows == 1):
        ...
        return rows
    if (rows >= 1):
        total_rows = rows + triangle(rows - 1)
        return total_rows
...
def countHi(str):
    if (len(str) == 0):
        ...
        return length
    if (str[0:2] == 'hi'):
        length = countHi(str[1:]) + 1
        return length


    length = countHi(str[1:])
    return length
...
def changeXY(str):
    if (len(str) == 0):
        ...
        return length
    if (str[0] == 'x'): 
        length = 'y' + changeXY(str[1:]) 
        return length
    
    length =  str[0] + changeXY(str[1:])
    return length
...
def changePi(str):
    ...
    if (str[:2] == 'pi'):
        string = '3.14' + changePi(str[2:])
        return string

    string = str[:1] + changePi(str[1:])
    return string
...
def array11(nums, index):
    ...
    if (len(nums) == index):
        ...
        num_times = array11(nums, index + 1) + int(nums[index] == 11)
        return num_times
...
def endX(str):
    if (len(str) <= 1):
        return str
    if (str[0] == 'x'):
        final_x = endX(str[1:]) + str[0]
        return final_x
    final_x = str[0] + endX(str[1:])
    return final_x
...
def countPairs(str):
    if (len(str) <= 2):
        ...
        return count
    if (str[0] == str[2]):
        count = countPairs(str[1:]) + 1
        return count
    count = countPairs(str[1:])
    return count
...
def countAbc(str):
    ...
    if (str[:3] == 'abc' or str[:3] == 'aba'):
        count = countAbc(str[1:]) + 1
        return count
    count = countAbc(str[1:])
    return count
...
def stringClean(str):
    if (len(str) <= 1):
        return str
    if (str[0] == str[1]):
        string = stringClean(str[1:])
        return string
    string = str[0] + stringClean(str[1:])
    return string
...
def countHi2(str):
    if len(str) <= 1: 
        ...
        return count
    if (str[:2] == 'hi'): 
        count = countHi2(str[2:]) + 1
        return count
    if (str[1:3] == 'hi') and (str[0] != 'x'): 
        count = countHi2(str[3:]) + 1
        return count
    if (str[1:3] == 'hi') and (str[0] == 'x'): 
        count =  countHi2(str[3:])
        return count
    count = countHi2(str[1:])
    return count
...
def parenBit(str):
    if (len(str) <= 1) or (str[0] == '(' and str[-1] == ')'):
        return str
    if (str[0] == '('):
        if (str[-1] == ')'):
            return str
...
def strCount(str, sub):
    if str[:len(sub)] == sub:
        string = strCount(str[len(sub):],sub) + 1
        return string
    if len(str) == 0: 
        length = 0
        return length
...
def strCopies(str, sub, n):
    if (str[:len(sub)] == sub):
        string = strCopies(str[1:],sub, n-1)
        return string
    if len(str) == 0: 
        appears = False
        return appears
...
def strDist(str, sub):
    if str[:len(sub)] == sub and str[-len(sub):] == sub:
        return len(str)
    if str[:len(sub)] == sub:
        length = strDist(str[:len(str)-1], sub)
        return length
    if str[-len(sub):len(str)-1] == sub:
        length = strDist(str[1:], sub)
        return length
    if len(str) < len(sub): 
        return 0
